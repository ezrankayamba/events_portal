        ...
        if data['trans_id'].strip() == 'ON-NET':
            ...
            if payment:
                ...
                if payment.ticket_issued:
                    ...
                    payment.save()
                    messages.success(
                        self.request, f'Success, you have successfully issued {count} ticket(s)!')
            else:
                messages.warning(
                    self.request, 'Failed, review your inputs and resubmit!')
...
def export_payments(request):
    resource = PaymentResource(request.user)
    dataset = resource.export()
    response = HttpResponse(dataset.csv, content_type='text/csv')
    export_id = datetime.now().strftime("%Y%m%d%H%M%S")
    response['Content-Disposition'] = f'attachment; filename="{export_id}_Payments.csv"'
    return response


def export_tickets(request):
    resource = TicketResource(request.user)
    dataset = resource.export()
    response = HttpResponse(dataset.csv, content_type='text/csv')
    export_id = datetime.now().strftime("%Y%m%d%H%M%S")
    response['Content-Disposition'] = f'attachment; filename="{export_id}_Tickets.csv"'
    return response
